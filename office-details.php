...
function fetch_office(str)
{
	
	//alert(str+"subdiv"+	str.substr(0,4));
	<?php
	if(isset($_REQUEST['officeid']))
	{
		?>
		alert("Police Station can't be changed while modify");
		bind_all();
		return false;
		<?php
	}
	...
	xmlhttp2.open("GET","ajaxfun.php?subdiv="+str.substr(0,4)+"&pol="+str+"&ofcid=n",true);
	xmlhttp2.send();
}
...
if($action=='Save')
{
	...
	$ddocode=clean_spl($_POST['ddocode']);
    ...
    $offcid=isset($_REQUEST['officeid'])?decode($_REQUEST['officeid']):"";
	$dist_code=$dist_cd;
	$usercd=$user_cd;
	if($offcid=='')
	{
		 $rsofc=fatch_office_maxcode($PoliceStation);	
	     $rowofc=getRows($rsofc);
			if($rowofc['officecd']==NULL)
			{
				if ($Subdivision!="0000")
					$OfficeID=$PoliceStation."0001";
				else
					$OfficeID="";
			}
			else
			{
				...
				$OfficeID=$PoliceStation.substr($inc,-4);
			}
	}
	//echo $OfficeID.$Subdivision."--sub--".$subdiv_cd;
	$cnt=check_duplicate_entry($officename,$designationOic,$Mb_no,$offcid);  
	if($cnt==0)
	{
		if($Subdivision)
		{
			...
			if(isset($_REQUEST['officeid']))
			{
				...
				$ret=update_officedetails($offcid,$designationOic,$officename,$ddocode,$Street,$Town,$PostOffice,$Pincode,$Municipality,$PoliceStation,$Statusofoffice,$email,$Ph_no,$Mb_no,$FAX_no,$ExistingStaff,$MaleStaff,$FemaleStaff,$Subdivision,$dist_code,$Natureofoffice,$usercd,$posted_date);
				if($ret==1)
				{
					redirect("office-details.php?msg=success");
				}
			}
			else
			{
				$ret=save_officedetails($OfficeID,$designationOic,$officename,$ddocode,$Street,$Town,$PostOffice,$Pincode,$Municipality,	$PoliceStation,$Statusofoffice,$email,$Ph_no,$Mb_no,$FAX_no,$ExistingStaff,$MaleStaff,$FemaleStaff,$Subdivision,$dist_code,$Natureofoffice,$usercd);
			}
			//echo "return ".ret;
			if($ret==1)
			{
...
<?php	if(isset($_REQUEST['officeid']))
	...
	var ddocode=document.getElementById('ddocode');
	ddocode.value="<?php echo $rowOffice['ddocode']; ?>";
...
<tr><td align="center"><form method="post" name="form1" id="form1">
  <table width="95%" class="form" cellpadding="0">
    ...
    <tr>
      ...
      <td align="left"> <select name="Subdivision" id="Subdivision" style="width:150px;" onchange="fatch_block(this.value)" <?php  if($subdiv_cd != 0) {?> disabled="disabled" <?php } ?>>
     ...
     <tr>
      ...
      <td align="left"><span id='Police_Station'><select name="PoliceStation" id="PoliceStation" style="width:150px;" onchange="fetch_office(this.value)">
      <?php
	  	if(isset($offccd))
		{
	...
	<tr><td align="left"><span class="error">&nbsp;&nbsp;</span>Office ID</td><td align="left"><span id="ofcid"><input type="text" name="OfficeID" id="OfficeID" style="width:142px;" readonly="true" /></span></td><td colspan="2">&nbsp;</td></tr>
    <tr>
