...
if($action=='Save')
{
	...
	if(isset($_REQUEST['venue_cd']))
	{
		...
		$ret=update_trainingvenue($venue_cd,$subdiv_cd,$venuename,$venueaddress1,$venueaddress2,$maximumcapacity,$usercd,$posted_date);
		if($ret==1)
		{
			?> <script>location.replace("trainingvenue.php?msg=success");</script> <?php
		}
    }
	else
	{
	
	$ret=save_trainingvenue($venue_cd,$subdiv_cd,$venuename,$venueaddress1,$venueaddress2,$maximumcapacity,$usercd);
	}
	if($ret==1)
	{
