...
$phase=(isset($_REQUEST['phase'])?decode($_REQUEST['phase']):'0');
...
$rsApp=first_app_letter3_print1_extra($subdiv,$phase,$from-1,$to-$from+1);
    ...
    for($i=1;$i<=rowCount($data);$i++)
	{
		...
		//	$this->SetTextColor(0,0,0);
			...
			for($l=0;$l<count($header2);$l++)
				$this->Cell($w2[$l],7,$header2[$l],'LTR',0,'C',true);	 
			$this->Ln();
			...
			$this->MultiCell($w2[0],6,'kjjjjjjjjjjjjjjjlllll ghgh tht','LTR','L',$fill);		
			$this->SetXY($x + 23, $y);
			$this->MultiCell($w2[1],6,$row['venuename']." we dont know the current order  ".$row['venueaddress'],'LTR','J',$fill);
			$this->SetXY($x + 160, $y);
			$this->MultiCell($w2[2],6,$row['training_dt'],'LTR','J',$fill);
	//$this->SetXY($x + 190, $y);
		/*		$this->Cell($w2[0],6,$row['training_desc'],'LTR',0,'L',$fill);						
			$this->Cell($w2[1],6,$row['venuename']." ".$row['venueaddress'],'LTR',0,'L',$fill);
