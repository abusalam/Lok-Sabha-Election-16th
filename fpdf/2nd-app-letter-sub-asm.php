  ...
  //  $this->SetTextColor(0);
    ...
    for($i=1;$i<=rowCount($data);$i++)
	{
		...
		if($count<$per_page)
	    {
			...
			$sql_mo="select * from second_appt";
			$mo=execSelect($sql_mo);
			$row5=getRows($mo);
		...
		//	$this->SetTextColor(0,0,0);
			...
			$mo_name="1.   ".$row5['mo_name'];
			$mo_desig=$row5['mo_designation'];
			$mo_code=$row5['mo_personcd'];
			$mo_office=$row5['mo_officename'];
			$mo_ofc_address="Office (".$row5['mo_officecd'].") : ".$row5['mo_officename'].", ".$row5['mo_officeaddress'];
			$mo_ofc_address1="P.O. - ".$row5['mo_postoffice'].", Subdiv. - ".$row5['mo_subdivision'].", ".$row5['district'];
			$mo_ofc_cd="OFFICE - (".$row5['mo_officecd'].")";
			$mo_post_stat=$row5['mo_post_stat'];
