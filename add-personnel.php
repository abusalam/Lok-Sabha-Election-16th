...
function ofc_ser(str)
{
	office=document.getElementById("offcode");
}
	  ...
	  if (xmlhttp1.readyState==4 && xmlhttp1.status==200)
		{
			if(xmlhttp1.responseText.length!=0)
				document.getElementById("personcd").innerHTML=xmlhttp1.responseText;
			else
				document.getElementById("personcd").innerHTML="<input type='text' name='p_id' id='p_id' style='width:142px;' readonly='readonly' />";
		}
	  }*/
...
function fatch_branch(str)
{
	...
	xmlhttp.open("GET","ajaxfun.php?bank="+str,true);
	xmlhttp.send();
}
...
function validate()
{
	...
	if(remarks=="0")
	{
		document.getElementById("msg").innerHTML="Select Remarks";
	...
	if(!dob1.match(/^(?:(0[1-9]|[12][0-9]|3[01])[\- \/.](0[1-9]|1[012])[\- \/.](19|20)[0-9]{2})$/))
	{
...
if($action=='Save')
{
	...
	if($p_id=='')
	{
		...
		if($rowmaxcode[0]==null)
			$p_id=$dist_code.$subdiv_cd."000001";
		else
			$p_id=$rowmaxcode[0]+1;*/
	    $police_cd=fatch_offcDtl_policestn_cd($offcode);
		//echo $subdiv_cd.$police_cd;
		$rsmaxcode=fatch_personnel_maxcode($subdiv_cd,$police_cd);
		
		$rowmaxcode=getRows($rsmaxcode);
		
		if($rowmaxcode[0]==null||$rowmaxcode[0]==0)
			$p_id=$police_cd."00001";
		else
		{
			$temp= $rowmaxcode[0];
			$inc = 100000+substr($temp,-5)+1;
			$p_id=$police_cd.substr($inc,-5);
			//$p_id=$rowmaxcode[0]+1;
		}
	}
...
var ddlText, ddlValue, ddl, lblMesg;
    ...
    function FilterItems(value) {
	  ooo = document.getElementById("ofc_ser").value;
		//alert(ooo.length);
	  if(ooo.length>8 || ooo.length==0)
	  {
            ddl = document.getElementById("offcode");
...
<tr><td align="center"><form method="post" name="form1" id="form1" enctype="multipart/form-data">
  <table width="95%" class="form" cellpadding="0">
    ...
    <tr>
      <td align="center">&nbsp;</td><td align="left" colspan="3"><input type="text" name="ofc_ser" id="ofc_ser" onkeyup="return FilterItems(this.value);" maxlength="10" /></td>
    </tr>
    <tr height="40px">
    ...
    <tr>
      <td align="left"><span class="error">*</span>Name of Employee</td>
      ...
      <td align="left"><input type="text" name="designation" id="designation" style="width:142px;" /></td>
    </tr>
    <tr>
      <td align="left"><span class="error">*</span>Date of Birth</td>
      <td align="left"><input type="text" name="dob" id="dob" maxlength="10" style="width:142px;" autocomplete="off" placeholder="DD/MM/YYYY"
    onkeyup="
        var v = this.value;
        if (v.match(/^\d{2}$/) !== null) {
            this.value = v + '/';
            ...
            this.value = v + '/';
        }" onchange="return date_check(this.value);" /></td>
    ...
    <tr>
      ...
      <td align="left"><select name="bank" id="bank" style="width:150px;" onchange="fatch_branch(this.value)">
      						<option value="0">-Select Bank Name-</option>
                            <?php 	$rsBn=fatch_bank($dist_cd);
    ...
    <tr>
      ...
      <td align="left"><select name="ac_per" id="ac_per" style="width:150px;">
      						<option value="0">-Select Assembly-</option>
							<?php 	$rsAsPer=fatch_assembly_all();
      ...
      <td align="left"><select name="ac_posting" id="ac_posting" style="width:150px;">
      						<option value="0">-Select Assembly-</option>
							<?php 	$rsAsPos=fatch_assembly_all();
    ...
    <tr>
      ...
      <td align="left"><select name="pgroup" id="pgroup" style="width:150px;" >
...
<?php if(!isset($_REQUEST['personcd']))
     {
	   if(!isset($_SESSION['ofcname'])) {?>
         offcode.value = "0";
<?php } else {?>
offcode.value = "<?php echo $_SESSION['ofcname']; ?>";
fatch_offcdtl("<?php echo $_SESSION['ofcname']; ?>");
<?php }
	 }else{
	$_SESSION['ofcname']="";
	unset($_SESSION['ofcname']);
}?>
