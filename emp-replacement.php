...
function new_per_search()
{
	...
	xmlhttp.open("GET","ajax-replacement.php?for_subdiv="+for_subdiv+"&forpc="+forpc+"&assembly="+forassembly+"&posting_status="+posting_status+"&groupid="+groupid+"&gender="+gender+"&ofc_cd="+draft_subdiv+"&hid_ofccd="+ofc_cd+"&opn=g_new_per",true);
	xmlhttp.send();
}
			...
			else
			{
				  document.getElementById('fakecontainer').style.display= 'none';
				   alert("Dulplicate record not allowed.Please try aagain");
					...
					location.replace("emp-replacement.php");
