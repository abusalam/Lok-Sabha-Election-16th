...
$par=0;
	...
	if($this->result->num_rows > 0) {
		while($row = $this->result->fetch_assoc()) {
			 ...
			 $pst=$row['poststat']	;
			 $bk=$row['booked']	;
			 if ((strcmp($pst,'MO')==0) and (strcmp($bk,'P')==0) ){
			 
			 	$prp=$prp+1;
			 }
			 if ((strcmp($pst,'MO')==0) and (strcmp($bk,'R')==0) ){
	...
	echo ' <br />';
       ...
       if (($totreq-($prp))>0) {
		   ...
		   print 'Total MO requirement : ' .$totreq;
		...
		{
			print 'Total MO requirement (excluding Reserve) : ' .$totreq;
			echo ' <br>';
		echo ' <br>';
		}
		print 'Booked Status :-> ';
