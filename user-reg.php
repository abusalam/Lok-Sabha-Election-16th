...
function change_category(cat)
{
	if(cat=="District") {
		...
		document.getElementById('trAssembly').style.visibility="hidden";
		return false; }
	else
	...
	if(cat=="Assembly") {
		document.getElementById('trAssembly').style.visibility="visible";
		document.getElementById('trDist').style.visibility="visible";
		document.getElementById('trSubdiv').style.visibility="visible";
		return false; }
	else
...
function subdiv_change(str)
{
	...
	if(category=="Assembly")
	{
		...
		xmlhttp.onreadystatechange=function()
		  ...
		  if (xmlhttp.readyState==4 && xmlhttp.status==200)
			{
			document.getElementById("am_result").innerHTML=xmlhttp.responseText;
			}
		  }
		xmlhttp.open("GET","ajax-user.php?subdiv="+str+"&opn=assembly",true);
		xmlhttp.send();
	}
...
function validate()
{
	...
	if(category=="Assembly")
	{
		...
		if(document.getElementById("assembly").value=="0" || document.getElementById("assembly").value=="")
		{
...
if($action=='Save')
{
	$district_cd=''; $subdiv=''; $parliament=''; $assembly_cd='';
	$userid=$_POST['userid'];
	...
	if($c_user==0)
	{
		...
		if(isset($_REQUEST['user_cd']))
		{
			$ret=update_user(decode($_REQUEST['user_cd']),$password,$category,$district_cd,$subdiv,$parliament,$assembly_cd);
			delete_permission(decode($_REQUEST['user_cd']));
		}
		else
		{
			$ret=save_user($userid,$password,$category,$district_cd,$subdiv,$parliament,$assembly_cd); 
		}
	...
	unset($userid,$password,$category,$district_cd,$subdiv,$parliament,$c_user,$ret,$assembly_cd);
}
?>
<?php
if(isset($_REQUEST['user_cd']))
...
<?php	if(isset($_REQUEST['user_cd']))
	...
	if(category.value=='District' || category.value=='Sub-Division' || category.value=='Parliament' || category.value=='Assembly')
	{
		document.getElementById('trDist').style.visibility="visible";
	...
	if(category.value=='Sub-Division' || category.value=='Parliament' || category.value=='Assembly')
	{
		document.getElementById('trSubdiv').style.visibility="visible";
...
if(isset($user_cd))
{
	if($usr_cat=='Sub-Division' || $usr_cat=='Parliament' || $usr_cat=='Assembly')
	{
		$dist=$usr_dist;
...
?>
    ...
    <tr id="trAssembly" style="visibility:hidden;"><td align="left"><span class="error">*</span>Assembly</td><td align="left" id="am_result"><select name="assembly" id="assembly" style="width:200px;">
<?php
if(isset($user_cd))
{
	if($usr_cat=='Assembly')
	{
		$subdiv=$usr_sub;
		$rsAM=fatch_assembly($subdiv);
		$num_rows=rowCount($rsAM);
		if($num_rows>0)
		{
			...
			{
				$rowAM=getRows($rsAM);
				echo "<option value='$rowAM[0]'>$rowAM[2]</option>\n";
			}
		}
		unset($rsAM,$num_rows,$rowAM);
	}
}
?>
</select></td>
    ...
    if($num_rows>0)
    {
        ...
        for($i=1;$i<=$num_rows;$i++)
        {
			 ...
			 $user_cd=isset($_REQUEST['user_cd'])?decode($_REQUEST['user_cd']):"";
			 
			 $rsPer=get_user_permission_list($menu,'',$user_cd);
			 if(rowCount($rsPer)>0)
			 {
					$rowPer=getRows($rsPer);
					$m_menu=$rowPer['menu_cd'];
			 }
			 else
			 {
				 $m_menu=0;
			 }
            echo "<tr><td align='left'><input type='checkbox' id='chkmenu_$i' name='chkmenu_$i' onclick='group_menu_row(this,$menu);' ";
			if($menu==$m_menu) echo 'checked';
			echo "/><label for='chkmenu_$i'>".$rowMenu['menu']."</label><input type='hidden' name='hidmenu_$i' id='hidmenu_$i' value='$menu' /></td>\n";
			echo "<td align='left'>";
            ...
            if($num_rows_submenu>0)
            {
                ...
                for($j=1;$j<=$num_rows_submenu;$j++)
                {
					...
					$submenu=$rowSubmenu['submenu_cd'];
					 ...
					 $rsSub=get_user_permission_list('',$submenu,$user_cd);
					 if(rowCount($rsSub)>0)
					 {
							$rowSub=getRows($rsSub);
							$s_menu=$rowSub['submenu_cd'];
					 }
					 else
					 {
						 $s_menu=0;
                    ...
                    echo "<input type='checkbox' id='chksubmenu_$i$j' name='chksubmenu_$i$j' onclick='group_submenu_row(this,$menu);' ";
					if($submenu==$s_menu) 
					{
					  echo 'checked';
					  $count++;
					}
					echo "/><label for='chksubmenu_$i$j'>".$rowSubmenu['submenu']."</label><input type='hidden' name='hidsubmenu_$i$j' id='hidsubmenu_$i$j' value='$submenu' /><br />\n";
                }
				echo "<input type='hidden' name='hid_subtotal_$i' id='hid_subtotal_$i' value='$count'/>";
            }
            echo "</td></tr>\n";
        }
    }
...
<script>
/*************************for menu change*****************************/
function group_menu_row(ID,CD)
{ 
    if(ID.checked)
	{
		var totalsm=document.getElementById('hid_submenucount_'+CD).value;
		document.getElementById('hid_subtotal_'+CD).value=totalsm;
		for(i=1;i<=totalsm;i++)
		{
			//alert(CD+""+i);
			var submenu=document.getElementById('chksubmenu_'+CD+""+i);
	...
	else
	{
		...
		document.getElementById('hid_subtotal_'+CD).value=0;
	}
}
/************************for submenu change***********************/
function group_submenu_row(ID,CD)
{
	Count=0;
	var hid_subtotal=document.getElementById('hid_subtotal_'+CD).value;
    if(ID.checked)
	{
		if(Count==hid_subtotal)
		{
		       Count++;
		       document.getElementById('hid_subtotal_'+CD).value=Count;	
		}
		else
		{
			hid_subtotal++;		
			document.getElementById('hid_subtotal_'+CD).value=hid_subtotal;
		}
		var chkmenu=document.getElementById('chkmenu_'+CD);
	...
	else
	{
		if(hid_subtotal!=1)
		{
		  document.getElementById('hid_subtotal_'+CD).value=hid_subtotal-1;
		}
		else
		{
			 document.getElementById('hid_subtotal_'+CD).value=0;
