...
// select QUERY ON A assembly TABLE
 	...
 	$this->sobj= new mysqliconn();
		...
		if (strcmp($stat,'S'==0)) {
	        $this->result = $this->msqli->query("SELECT assemblycd,pccd,no_of_member,no_party,subdivisioncd FROM `assembly_party` where subdivisioncd='$sub' order by RandOrder") or die($this->msqli->error.__LINE__);
			}
