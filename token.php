	...
	if($this->result1->num_rows > 0) {
		...
		$k=0;
		 while ($row1=$this->result1->fetch_assoc()) {
		    $pst=$row1['poststat'];
			$tkn=0;
			$this->result = $this->msqli->query("SELECT * FROM `training_pp` where for_subdivision='$subdiv' and training_type='$trgtype' and post_stat='$pst' order by training_sch") or die($this->msqli->error.__LINE__);
